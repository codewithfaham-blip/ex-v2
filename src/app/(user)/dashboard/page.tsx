  ...
  return (
    ...
    <div className="p-4 md:p-8 lg:p-10 pt-24 lg:pt-10 max-w-[1400px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
        ...
        {/* Balance Card */}
        <div className="bg-zinc-900/40 border border-zinc-800/50 p-6 rounded-[2.5rem] relative overflow-hidden group shadow-2xl">
          <div className="absolute -top-24 -right-24 w-64 h-64 bg-blue-600/10 rounded-full blur-[80px]" />
          <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest mb-3 italic">Liquidity Balance</p>
          <h3 className="text-4xl font-black text-white tracking-tighter">${user.balance.toFixed(2)}</h3>
          <div className="mt-6 flex items-center gap-2">
             <Activity size={12} className="text-emerald-500" />
             <span className="text-emerald-500 text-[8px] font-black uppercase tracking-widest">Live Portfolio Flowing</span>
          </div>
        </div>

        {/* Deposits Card */}
        <div className="bg-zinc-900/40 border border-zinc-800/50 p-6 rounded-[2.5rem] flex flex-col justify-between group hover:border-blue-600/30 transition-all min-h-[160px]">
          <div className="flex justify-between items-start">
            <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest italic">Injected Assets</p>
            <ArrowDownCircle className="text-blue-500 group-hover:rotate-12 transition-transform" size={20} />
          </div>
          <h3 className="text-2xl font-black text-white">${totalDeposits.toFixed(2)}</h3>
        </div>

        {/* Withdrawals Card */}
        <div className="bg-zinc-900/40 border border-zinc-800/50 p-6 rounded-[2.5rem] flex flex-col justify-between group hover:border-emerald-600/30 transition-all min-h-[160px]">
          <div className="flex justify-between items-start">
            <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest italic">Total Extractions</p>
            <ArrowUpCircle className="text-emerald-500 group-hover:-rotate-12 transition-transform" size={20} />
          </div>
          <h3 className="text-2xl font-black text-emerald-500">${totalWithdrawals.toFixed(2)}</h3>
        </div>
      </div>
