                ...
                {user.deposits.slice(0, 5).map((dep) => (
                  <div key={dep.id} className="flex items-center justify-between p-6 bg-zinc-950/50 border border-zinc-900 rounded-[1.5rem] hover:border-zinc-700 transition-all group">
                    <div className="flex items-center gap-5">
                       ...
                       <div>
                          <p className="text-[11px] font-black uppercase tracking-tight text-white">{dep.planName || 'Asset Injection'}</p>
                          <p className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest mt-1 italic">{dep.status}</p>
                       </div>
                    </div>
