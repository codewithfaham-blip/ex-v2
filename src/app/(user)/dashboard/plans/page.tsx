...
const iconMap: Record<string, any> = {
  ...
  Crown: <Crown className="text-purple-500" />,
};
    ...
    <div className="p-4 md:p-8 pt-24 lg:pt-10 max-w-7xl mx-auto min-h-screen space-y-12">
      ...
      <div className="space-y-6">
        ...
        {fetchingPlans ? (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-64 bg-zinc-900/30 border border-zinc-800 rounded-[2rem] animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan, i) => (
              <div key={i} className={`relative bg-zinc-900/30 border ${plan.popular ? 'border-blue-600/50 shadow-[0_0_30px_-10px_rgba(37,99,235,0.3)]' : 'border-zinc-800/50'} p-8 rounded-[2.5rem] flex flex-col group hover:scale-[1.02] transition-transform`}>
                ...
                <div className="bg-zinc-950 p-4 w-fit rounded-2xl border border-zinc-800 mb-6 group-hover:border-blue-500/30 transition-colors">
                  {iconMap[plan.icon] || <Zap className="text-blue-500" />}
                </div>
