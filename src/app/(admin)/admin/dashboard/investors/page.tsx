    ...
    <div className="p-4 md:p-10 pt-24 lg:pt-10 max-w-[1600px] mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.3em] mt-2 italic">Directory of all verified system nodes</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-64">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input placeholder="Search Node ID..." className="w-full bg-white border border-slate-200 rounded-2xl py-3 pl-12 pr-4 text-xs focus:outline-none focus:border-purple-600 transition-all shadow-sm"/>
          </div>
        ...
        {investors.map((investor) => (
          <div key={investor.id} className="bg-white border border-slate-200 p-6 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6 hover:border-purple-600/30 transition-all group shadow-sm">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-8 w-full md:w-auto flex-1 px-4 text-center md:text-left">
              <div>
                <p className="text-[9px] text-slate-400 font-black uppercase tracking-tighter mb-1">Current Liquidity</p>
                <p className="text-lg font-black text-slate-900">Rs. {investor.balance.toFixed(2)}</p>
              </div>
              <div>
                <p className="text-[9px] text-slate-400 font-black uppercase tracking-tighter mb-1">Total Injected</p>
                <p className="text-lg font-black text-slate-400">Rs. {investor.deposits.reduce((acc, curr) => acc + curr.amount, 0).toFixed(2)}</p>
              </div>
              <div className="hidden md:block">
                <p className="text-[9px] text-slate-400 font-black uppercase tracking-tighter mb-1">Node Security</p>
                <span className="bg-emerald-50 text-emerald-600 px-3 py-1 rounded-full text-[9px] font-black uppercase italic border border-emerald-100">Active Node</span>
              </div>
            </div>
